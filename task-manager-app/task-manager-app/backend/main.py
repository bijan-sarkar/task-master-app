...
def get_or_create_valid_user(user_id: str, db: Session) -> models.User:
    """Ensures a task is always associated with a real user to prevent foreign key errors."""
    user = db.query(models.User).filter(models.User.id == user_id).first()
    if user:
        return user
    
    # Try finding by default email or take first existing user
    ...
    new_user = models.User(
        email="default@taskmaster.dev",
        name="Bijon Sarkar",
        accountability_tone="harsh"
    )
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
...
def get_user(user_id: str, db: Session = Depends(get_db)):
    user = get_or_create_valid_user(user_id, db)
    return user
...
def set_availability(user_id: str, slots: List[schemas.AvailabilitySlotCreate], db: Session = Depends(get_db)):
    user = get_or_create_valid_user(user_id, db)
    db.query(models.AvailabilitySlot).filter(models.AvailabilitySlot.user_id == user.id).delete()
    for s in slots:
...
def get_availability(user_id: str, db: Session = Depends(get_db)):
    user = get_or_create_valid_user(user_id, db)
    return db.query(models.AvailabilitySlot).filter(models.AvailabilitySlot.user_id == user.id).all()
...
def trigger_task_scheduling(user_id: str, db: Session = Depends(get_db)):
    user = get_or_create_valid_user(user_id, db)
    user_slots = db.query(models.AvailabilitySlot).filter(models.AvailabilitySlot.user_id == user.id).all()
    if not user_slots:
...
def get_dashboard_data(user_id: str, db: Session = Depends(get_db)):
    user = get_or_create_valid_user(user_id, db)
    tasks = db.query(models.Task).filter(models.Task.user_id == user.id).order_by(models.Task.created_at.desc()).all()
    total = len(tasks)
