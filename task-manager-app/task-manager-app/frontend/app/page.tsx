...
export default function Dashboard() {
  ...
  async function handleSaveTask(e: React.FormEvent) {
    ...
    const currentTitle = taskForm.title;
    const currentDesc = taskForm.description;
    const currentMins = taskForm.estimated_minutes;
    const currentPrio = taskForm.priority;
    const currentStatus = taskForm.status;

    // 1. Optimistic UI update: show task immediately
    ...
    if (editingTask) {
      ...
      setData((prev: any) => ({
        ...
        all_tasks: [newTaskObj, ...(prev?.all_tasks || [])],
